<?php
session_start();

if(!isset($_SESSION['sess_user_id']) && $_SESSION['sess_user_id'] == "") 
{
  header('location:admin_login.php');
}
?>
<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="utf-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1, maximum-scale=1, shrink-to-fit=no">
    <title>CORK Admin Template - Tables UI Kit</title>
    <link rel="icon" type="image/x-icon" href="assets/img/favicon.ico" />
    <!-- BEGIN GLOBAL MANDATORY STYLES -->
    <link href="bootstrap/css/bootstrap.min.css" rel="stylesheet" type="text/css" />
    <link href="assets/css/plugins.css" rel="stylesheet" type="text/css" />
    <!-- END GLOBAL MANDATORY STYLES -->
    <!-- BEGIN PAGE LEVEL CUSTOM STYLES -->
    <link rel="stylesheet" type="text/css" href="assets/css/forms/theme-checkbox-radio.css">
    <link href="assets/css/tables/table-basic.css" rel="stylesheet" type="text/css" />
    <link href="assets/css/perfect-scrollbar.css" rel="stylesheet" type="text/css" />
    <link rel="stylesheet" href="css/style.css">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css">
    <!-- END PAGE LEVEL CUSTOM STYLES -->

    <script type="text/javascript">

        function validateChangeUserPwd() {
            if (document.getElementById('update_change_userpwd').checked) {
                document.getElementById('update_confirm_userpwd').disabled = false;
                document.getElementById('update_userpwd').disabled = false;
                document.getElementById('update_change_userpwd').value = 1;
            } else {
                document.getElementById('update_confirm_userpwd').disabled = true;
                document.getElementById('update_userpwd').disabled = true;
                document.getElementById('update_change_userpwd').value = 0;
            }
        }
        function checkUpdateIsActive() {
            if (document.getElementById('update_is_active').checked) {
                document.getElementById('hidden_update_is_active').value = 1;
            }
            else {
                document.getElementById('hidden_update_is_active').value = 0;
            }
        }
        function checkIsActive() {
            if (document.getElementById('is_active').checked) {
                document.getElementById('hidden_is_active').value = 1;
            }
            else {
                document.getElementById('hidden_is_active').value = 0;
            }
        }
        function validateChangePwd() {
            if (document.getElementById('update_admin_change_password').checked) {
                document.getElementById('update_admin_confirm_password').disabled = false;
                document.getElementById('update_admin_password').disabled = false;
                document.getElementById('update_admin_change_password').value = 1;
            } else {
                document.getElementById('update_admin_confirm_password').disabled = true;
                document.getElementById('update_admin_password').disabled = true;
                document.getElementById('update_admin_change_password').value = 0;
            }

        }
    </script>

</head>

<body class="sidebar-noneoverflow" data-spy="scroll" data-target="#navSection" data-offset="140">

    <!--  BEGIN NAVBAR  -->
    <div class="header-container">
        <header class="header navbar navbar-expand-sm">

            <a href="javascript:void(0)" class="sidebarCollapse" data-placement="bottom"><svg
                    xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none"
                    stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
                    class="feather feather-menu">
                    <line x1="3" y1="12" x2="21" y2="12"></line>
                    <line x1="3" y1="6" x2="21" y2="6"></line>
                    <line x1="3" y1="18" x2="21" y2="18"></line>
                </svg></a>

            <div class="nav-logo align-self-center">
                <a class="navbar-brand" href="index.html"><img alt="logo" src="assets/img/90x90.jpg"> <span
                        class="navbar-brand-name">CORK</span></a>
            </div>

            <ul class="navbar-item flex-row mr-auto">
                <li class="nav-item align-self-center search-animated">
                    <form class="form-inline search-full form-inline search" role="search">
                        <div class="search-bar">
                            <input type="text" id="search_text" class="form-control search-form-control ml-lg-auto"
                                placeholder="Search...">
                        </div>
                    </form>
                    <svg onclick="searchStaff()" xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                        viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"
                        stroke-linejoin="round" class="feather feather-search toggle-search">
                        <circle cx="11" cy="11" r="8"></circle>
                        <line x1="21" y1="21" x2="16.65" y2="16.65"></line>
                    </svg>
                </li>
            </ul>

            <ul class="navbar-item flex-row nav-dropdowns">
                <li class="nav-item dropdown user-profile-dropdown order-lg-0 order-1">
                    <a href="javascript:void(0);" class="nav-link dropdown-toggle user" id="user-profile-dropdown"
                        data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                        <div class="media">
                            <img src="assets/img/90x90.jpg" class="img-fluid" alt="admin-profile">
                            <div class="media-body align-self-center">
                                <h6><span>Hi,</span> Admin</h6>
                            </div>
                        </div>
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none"
                            stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
                            class="feather feather-chevron-down">
                            <polyline points="6 9 12 15 18 9"></polyline>
                        </svg>
                    </a>
                    <div class="dropdown-menu position-absolute animated fadeInUp"
                        aria-labelledby="user-profile-dropdown">
                        <div class="">
                            <div class="dropdown-item">
                                <a class=""  data-toggle="modal" onclick="GetAdminDetails()"><svg xmlns="http://www.w3.org/2000/svg" width="24"
                                        height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                                        stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
                                        class="feather feather-user">
                                        <path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"></path>
                                        <circle cx="12" cy="7" r="4"></circle>
                                    </svg> My Profile</a>
                            </div>
                            <div class="dropdown-item">
                                <a class="" href="admin_login.php"><svg xmlns="http://www.w3.org/2000/svg" width="24"
                                        height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                                        stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
                                        class="feather feather-log-out">
                                        <path d="M9 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h4"></path>
                                        <polyline points="16 17 21 12 16 7"></polyline>
                                        <line x1="21" y1="12" x2="9" y2="12"></line>
                                    </svg> Sign Out</a>
                            </div>
                        </div>
                    </div>

                </li>
            </ul>
        </header>
    </div>
    <!--  END NAVBAR  -->

    <!--  BEGIN MAIN CONTAINER  -->
    <div class="container main-container">

        <div class="overlay"></div>
        <div class="search-overlay"></div>

        <!--  BEGIN TOPBAR  -->
        <div class="topbar-nav header navbar" role="banner">
            <nav id="topbar">
                <ul class="navbar-nav theme-brand flex-row  text-center">
                    <li class="nav-item theme-logo">
                        <a href="index.html">
                            <img src="assets/img/90x90.jpg" class="navbar-logo" alt="logo">
                        </a>
                    </li>
                    <li class="nav-item theme-text">
                        <a href="index.html" class="nav-link"> CORK </a>
                    </li>
                </ul>

                <ul class="list-unstyled menu-categories" id="topAccordion">

                    <li class="menu single-menu">
                        <a href="manage_users.php">
                            <div class="">
                                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
                                    fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"
                                    stroke-linejoin="round" class="feather feather-home">
                                    <path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"></path>
                                    <polyline points="9 22 9 12 15 12 15 22"></polyline>
                                </svg>
                                <span>Manage Users</span>
                            </div>
                        </a>
                    </li>

                    <li class="menu single-menu">
                        <a href="manage_announcements.php">
                            <div class="">
                                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
                                    fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"
                                    stroke-linejoin="round" class="feather feather-cpu">
                                    <rect x="4" y="4" width="16" height="16" rx="2" ry="2"></rect>
                                    <rect x="9" y="9" width="6" height="6"></rect>
                                    <line x1="9" y1="1" x2="9" y2="4"></line>
                                    <line x1="15" y1="1" x2="15" y2="4"></line>
                                    <line x1="9" y1="20" x2="9" y2="23"></line>
                                    <line x1="15" y1="20" x2="15" y2="23"></line>
                                    <line x1="20" y1="9" x2="23" y2="9"></line>
                                    <line x1="20" y1="14" x2="23" y2="14"></line>
                                    <line x1="1" y1="9" x2="4" y2="9"></line>
                                    <line x1="1" y1="14" x2="4" y2="14"></line>
                                </svg>
                                <span>Manage Announcements</span>
                            </div>
                        </a>
                    </li>

                    <li class="menu single-menu">
                        <a href="upload_statements.php">
                            <div class="">
                                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
                                    fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"
                                    stroke-linejoin="round" class="feather feather-box">
                                    <path
                                        d="M21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16z">
                                    </path>
                                    <polyline points="3.27 6.96 12 12.01 20.73 6.96"></polyline>
                                    <line x1="12" y1="22.08" x2="12" y2="12"></line>
                                </svg>
                                <span>Manage Statements</span>
                            </div>
                        </a>
                    </li>


                    <li class="menu single-menu">
                        <a href="file_management.php">
                            <div class="">
                                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
                                    fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"
                                    stroke-linejoin="round" class="feather feather-zap">
                                    <polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2"></polygon>
                                </svg>
                                <span>File Management</span>
                            </div>
                        </a>
                    </li>
                </ul>
            </nav>
        </div>
        <!--  END TOPBAR  -->
        <br><br>
        <!--  BEGIN CONTENT AREA  -->
        <div class="main-content">
            <div class="row">
                <div class="col-md-12">
                    <div class="pull-right">
                        <button class="btn btn-outline-primary mb-2" data-toggle="modal"
                            data-target="#add_new_record_modal">Add New Announcement</button>
                    </div>
                </div>
            </div>
            <br>
            <div class="row">
                <div class="col-md-12">

                    <div class="announcement_content table_responsible"></div>
                </div>
            </div>
        </div>
        <div class="footer-wrapper">
            <div class="footer-section f-section-1">
                <p class="">Copyright © 2020 <a target="_blank" href="https://designreset.com">DesignReset</a>, All
                    rights reserved.</p>
            </div>
            <div class="footer-section f-section-2">
                <p class="">Coded with <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                        viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"
                        stroke-linejoin="round" class="feather feather-heart">
                        <path
                            d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z">
                        </path>
                    </svg></p>
            </div>
        </div>
    </div>
    <!--  END CONTENT AREA  -->

    </div>

    
<div class="modal fade" id="update_admin_proifle" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                    <h4 class="modal-title" id="myModalLabel">Update Admin Profile</h4>
                </div>
                <div class="modal-body">
    
                      <div class="form-group">
                          <label for="update_admin_username">User Name</label>
                          <input type="text" class="form-control" id="update_admin_username"  disabled="disabled" placeholder="User Name">
                      </div>
    
                      <div class="form-group">
                          <label for="update_admin_change_password">Change Password?</label>
                          <input type="checkbox" onclick="validateChangePwd()" class="form-check-input" id="update_admin_change_password"  value="0">
                      </div>
                      <div class="form-group">
                          <label for="update_admin_password">Password</label>
                          <input type="password" class="form-control" id="update_admin_password"  disabled="disabled" placeholder="Password">
                      </div>
    
                      <div class="form-group">
                          <label for="update_admin_confirm_password">Confirm Password</label>
                          <input type="password" class="form-control" id="update_admin_confirm_password"  disabled="disabled" placeholder="Confirm Password">
                      </div>
    
                      <div class="form-group">
                          <label for="update_admin_first_name">First Name</label>
                          <input type="text" class="form-control" id="update_admin_first_name" placeholder="First Name">
                      </div>
    
                      <div class="form-group">
                           <label for="update_admin_last_name">Last Name</label>
                            <input type="text" class="form-control" id="update_admin_last_name" placeholder="Last Name">
                      </div>
    
                      <div class="form-group">
                          <label for="update_admin_email_address">Email Address</label>
                          <input type="email" class="form-control" id="update_admin_email_address" placeholder="Email Address">
    
                      </div>
    
    
                      <div class="form-group">
                          <label for="update_admin_mobile_number">Mobile Number</label>
                          <input type="text" class="form-control" id="update_admin_mobile_number" placeholder="Mobile Number">
                      </div>
    
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-outline-warning mb-2" data-dismiss="modal">Cancel</button>
                    <button type="button" class="btn btn-outline-primary mb-2" onclick="UpdateAdminProfile()">Save Changes</button>
                    <input type="hidden" id="hidden_admin_id">
                </div>
            </div>
        </div>
    </div>

    <div class="modal fade" id="add_new_record_modal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span
                            aria-hidden="true">&times;</span></button>
                    <h4 class="modal-title" id="myModalLabel">Add New Announcement</h4>
                </div>
                <div class="modal-body">

                    <div class="form-group">
                        <label for="valid_from_date">Valid From:</label>

                        <div class='input-group date' id='valid_from_date1'>
                            <input type='text' class="form-control" id='valid_from_date' />
                            <span class="input-group-addon">
                                <span class="glyphicon glyphicon-calendar"></span>
                            </span>
                        </div>

                    </div>

                    <div class="form-group">
                        <label for="valid_to_date">Valid To:</label>

                        <div class='input-group date' id='valid_to_date1'>
                            <input type='text' class="form-control" id='valid_to_date' />
                            <span class="input-group-addon">
                                <span class="glyphicon glyphicon-calendar"></span>
                            </span>
                        </div>

                    </div>



                    <div class="form-group">
                        <label for="ann_title">Title</label>
                        <input type="text" class="form-control" id="ann_title" placeholder="Title">
                    </div>

                    <div class="form-group">
                        <label for="ann_content">Announcement Content</label>
                        <textarea class="form-control" id="ann_content" rows="4" placeholder="Content"></textarea>
                    </div>

                    <div class="form-group">
                        <div class="form-check form-check-inline">
                            <input class="form-check-input" type="radio" name="announcement_status" value="0"
                                id='announcement_status1' onclick="setAnnouncementStatus(0)">
                            <label class="form-check-label" for="announcement_status">Draft</label>
                        </div>
                        <div class="form-check form-check-inline">
                            <input class="form-check-input" type="radio" name="announcement_status" value="1"
                                id='announcement_status2' onclick="setAnnouncementStatus(1)">
                            <label class="form-check-label" for="announcement_status">Published</label>
                        </div>
                        <div class="form-check form-check-inline">
                            <input class="form-check-input" type="radio" name="announcement_status" value="2"
                                id='announcement_status3' onclick="setAnnouncementStatus(2)">
                            <label class="form-check-label" for="announcement_status">Hidden</label>
                        </div>
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-outline-warning mb-2" data-dismiss="modal">Cancel</button>
                    <button type="button" class="btn btn-outline-primary mb-2" onclick="addAnnouncement()">Save</button>
                    <input type="hidden" id="hidden_announcement_status">
                </div>
            </div>
        </div>
    </div>
    <!-- // Modal -->


    <!-- Modal - Update User details -->
    <div class="modal fade" id="update_announcement_modal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span
                            aria-hidden="true">&times;</span></button>
                    <h4 class="modal-title" id="myModalLabel">Update Announcement</h4>
                </div>
                <div class="modal-body">

                    <div class="form-group">
                        <label for="valid_from_date">Valid From:</label>

                        <div class='input-group date'>
                            <input type='text' class="form-control" id='update_valid_from_date' />
                            <span class="input-group-addon">
                                <span class="glyphicon glyphicon-calendar"></span>
                            </span>
                        </div>

                    </div>

                    <div class="form-group">
                        <label for="valid_to_date">Valid To:</label>

                        <div class='input-group date'>
                            <input type='text' class="form-control" id='update_valid_to_date' />
                            <span class="input-group-addon">
                                <span class="glyphicon glyphicon-calendar"></span>
                            </span>
                        </div>

                    </div>



                    <div class="form-group">
                        <label for="ann_title">Title</label>
                        <input type="text" class="form-control" id="update_ann_title" placeholder="Title">
                    </div>

                    <div class="form-group">
                        <label for="ann_content">Announcement Content</label>
                        <textarea class="form-control" id="update_ann_content" rows="4"
                            placeholder="Content"></textarea>
                    </div>

                    <div class="form-check form-check-inline">
                        <input class="form-check-input" type="radio" name="update_announcement_status"
                            id="update_announcement_status1" value="0" onclick="setUpdateAnnouncementStatus(0)">
                        <label class="form-check-label" for="update_announcement_status">Draft</label>
                    </div>
                    <div class="form-check form-check-inline">
                        <input class="form-check-input" type="radio" name="update_announcement_status"
                            id="update_announcement_status2" value="1" onclick="setUpdateAnnouncementStatus(1)">
                        <label class="form-check-label" for="update_announcement_status">Published</label>
                    </div>
                    <div class="form-check form-check-inline">
                        <input class="form-check-input" type="radio" name="update_announcement_status"
                            id="update_announcement_status3" value="2" onclick="setUpdateAnnouncementStatus(2)">
                        <label class="form-check-label" for="update_announcement_status">Hidden</label>
                    </div>

                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-outline-warning mb-2" data-dismiss="modal">Cancel</button>
                    <button type="button" class="btn btn-outline-primary mb-2" onclick="UpdateAnnouncement()">Save Changes</button>
                    <input type="hidden" id="hidden_announcement_id">
                    <input type="hidden" id="hidden_update_announcement_status">
                </div>
            </div>
        </div>
    </div>
    <!-- END MAIN CONTAINER -->

    <!-- BEGIN GLOBAL MANDATORY SCRIPTS -->
    <script src="assets/js/libs/jquery-3.1.1.min.js"></script>
    <script src="bootstrap/js/popper.min.js"></script>
    <script src="bootstrap/js/bootstrap.min.js"></script>
    <script src="js/script.js"></script>
    <script src="assets/js/perfect-scrollbar.min.js"></script>
    <script src="assets/js/app.js"></script>

    <script>
        $(document).ready(function () {
            App.init();
            readStaff();
        });
    </script>

    <script type="text/javascript">

        $(document).ready(function () {
            // READ records on page load
            readAnnouncements(); // calling function

        });

        $(function () {
            var bindDatePicker = function () {
                $(".date").datetimepicker({
                    format: 'YYYY-MM-DD',
                    icons: {
                        time: "fa fa-clock-o",
                        date: "fa fa-calendar",
                        up: "fa fa-arrow-up",
                        down: "fa fa-arrow-down"
                    }
                }).find('input:first').on("blur", function () {
                    // check if the date is correct. We can accept dd-mm-yyyy and yyyy-mm-dd.
                    // update the format if it's yyyy-mm-dd
                    var date = parseDate($(this).val());

                    if (!isValidDate(date)) {
                        //create date based on momentjs (we have that)
                        date = moment().format('YYYY-MM-DD');
                    }

                    $(this).val(date);
                });
            }

            var isValidDate = function (value, format) {
                format = format || false;
                // lets parse the date to the best of our knowledge
                if (format) {
                    value = parseDate(value);
                }

                var timestamp = Date.parse(value);

                return isNaN(timestamp) == false;
            }

            var parseDate = function (value) {
                var m = value.match(/^(\d{1,2})(\/|-)?(\d{1,2})(\/|-)?(\d{4})$/);
                if (m)
                    value = m[5] + '-' + ("00" + m[3]).slice(-2) + '-' + ("00" + m[1]).slice(-2);

                return value;
            }

            bindDatePicker();
        });
    </script>

</body>

</html>