...
/*					
        ...
        thzStickyHeader: function() {
            ...
            $(window).scroll(function(event) {
				...
				//var maxWidth 	= self.thzBodyContainerWidth( true ).newWidth;
		...
		thzSectionsScrollTo: function (){
			...
			var $add = $scrollList.attr('data-add');
			
			if($add){
				
				$add = JSON.parse($add);
				$.each($add,function(i,label){

					if(i == 'he' && label !=''){
						
						$('.thz-hero-section').attr('data-slabel',label);
					}
					...
					if(i == 'f' && label !=''){
						...
						if($('.thz-footer-reveal').length > 0){
							...
							$('.thz-footer-reveal').attr('data-slabel',label);
							
						}else{
							
							$('.thz-widgets-section').attr('data-slabel',label);
						}
			...
			$('[data-slabel]').each(function(index, element) {
                	
					
					var $label = $(this).attr('data-slabel'); 
					var $link = $(this).attr('id');
					
					var $li = $('<li/>')
						.appendTo($scrollList);
					var $a = $('<a/>')
						.addClass('thz-dot-nav-item thz-scroll')
						.attr('data-before', 0)
						.attr('data-duration', 800)
						.attr('href', '#'+$link)
						.appendTo($li);	
