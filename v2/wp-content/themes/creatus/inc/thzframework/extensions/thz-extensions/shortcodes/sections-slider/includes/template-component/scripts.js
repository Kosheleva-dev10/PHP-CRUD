(function($, localized){
	var eventsNamespace = '.templates-sections-slider',
		loadingId = 'fw-builder-templates-type-sections-slider',
		modal,
	...
	fwEvents.on('fw:option-type:builder:templates:init', function(data){
		...
		data.$elements.find('.fw-builder-templates-type-sections-slider')
			.off(eventsNamespace)
			.on('click'+ eventsNamespace, 'a[data-load-template]', function(){
				...
				$.ajax({
					...
					url: ajaxurl,
					data: {
						'action': 'fw_builder_templates_sections-slider_load',
			...
			.on('click'+ eventsNamespace, 'a[data-delete-template]', function(){
				...
				$.ajax({
					...
					url: ajaxurl,
					data: {
						'action': 'fw_builder_templates_sections-slider_delete',
						'builder_type': builder.get('type'),
						'template_id': templateId
					}
				})
	...
	fwEvents.on('fw:page-builder:shortcode:sections-slider:controls', function(data){
		data.$controls.prepend(
			$('<i class="fw-shortcode-sections-slider-save"></i>')
				...
				.on('click', function(e){
					...
					modal.on('change:values', function (modal, values) {
						...
						$.ajax({
							...
							url: ajaxurl,
							data: {
								'action': 'fw_builder_templates_sections-slider_save',
								'template_name': values.template_name,
								'sections-slider_json': JSON.stringify(data.model),
...
})(jQuery, _fw_option_type_builder_templates_sections_slider);