...
class FW_Ext_Builder_Templates_Component_Sections_Slider extends FW_Ext_Builder_Templates_Component
{
	...
	{
		return __('Sections slider', 'creatus');
	}
	...
	public function _render($data)
	{
		...
		if (empty($html)) {
			$html = '<div class="fw-text-muted">'. __('No Templates Saved', 'creatus') .'</div>';
		} else {
			$html =
				'<p class="fw-text-muted load-template-title">'. __('Load Template', 'creatus') .':</p>'
	...
	public function _enqueue($data)
	{
		...
		$uri = fw_ext('thz-extensions')->get_uri('/shortcodes/'. $this->get_type() .'/includes/template-component');
		$version = fw_ext('thz-extensions')->manifest->get_version();
		...
		wp_localize_script(
			'fw-option-builder-templates-'. $this->get_type(),
			'_fw_option_type_builder_templates_'. str_replace('-', '_', $this->get_type()),
			array(
				'l10n' => array(
					'template_name' => __('Template Name', 'creatus'),
					'save_template' => __('Save sections slider', 'creatus'),
					'save_template_tooltip' => __('Save as Template', 'creatus'),
				),
			)
		);
	}
