(function (fwe) {
	fwe.on('fw-builder:' + 'page-builder' + ':register-items', function (builder) {
		var PageBuilderThzSectionsItem,
			PageBuilderThzSectionsItemView,
		...
		PageBuilderThzSectionsItemView = builder.classes.ItemView.extend({
			...
			template: _.template(
				'<div class="pb-item-type-column pb-item custom-section sections-slider">' +
			...
			render: function () {
				...
				this.$el[this.model.get('fw-collapse') ? 'addClass' : 'removeClass']('pb-item-sections-slider-collapsed');

				/**
				 * Other scripts can append/prepend other control $elements
				 */
				fwEvents.trigger('fw:page-builder:shortcode:sections-slider:controls', {
			...
			editOptions: function (e) {
				 ...
				 * http://bit.ly/1KY6tpP
				 */
				fwEvents.trigger('fw:page-builder:shortcode:sections-slider:modal:before-open', {
					modal: this.modal,
					model: this.model,
					builder: builder,
					flow: flow
			...
			cloneItem: function (e) {
				...
				clonedColumn = new PageBuilderThzSectionsItem(attributes);
		...
		PageBuilderThzSectionsItem = builder.classes.Item.extend({
			defaults: {
				type: 'sections-slider'
			},
			initialize: function() {
				this.view = new PageBuilderThzSectionsItemView({
		...
		builder.registerItemClass(PageBuilderThzSectionsItem);
	});

	function itemData () {
		// return fw.unysonShortcodesData()['sections_slider'];
		return page_builder_item_type_sections_slider_data;
	}
})(fwEvents);
