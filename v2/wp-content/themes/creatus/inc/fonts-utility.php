...
function thz_build_typekit_list($data) {
	...
	foreach($kits_data as $k => $kitid){
		...
		$html .= '<span class="thz-typekit-name">'.esc_html('Kit name','creatus').': '.$kitid['name'].'</span>';
...
function thz_get_fontface_kits() {
	...
	$custom_kits =  apply_filters ('thz_filter_add_fontface_kits',array());
	
	
	if(!empty($custom_kits)){
		
		$kits = array_merge($kits, $custom_kits);	
