...
class Thz_Dynamic_Css {
	...
	public function thz_dynamic_css_init(){
		...
		$compileinline					= thz_get_theme_option ('thzopt/compileinline','donotcompile');
		$compilecss						= thz_get_theme_option ('thzopt/compilecss','donotcompile');
	 ...
	 */	
	public function thz_compile_inline_css($css,$page_fonts){
	...
	public function thz_remove_shortcodes_inline_css(){
		...
		if( function_exists('fw') && $this->thz_cached_file() ){
			...
			remove_action( 'wp_enqueue_scripts', array( fw()->extensions->get( 'shortcodes' ), '_action_enqueue_shortcodes_static_in_frontend_head' ), 30 );
	...
	public function _thz_action_enqueue_dynamic_scripts() {
		...
		if ( !empty( Thz_Doc::get('googleclassname' )) ) { 
		
			$all_google_class_names = Thz_Doc::get('googleclassname');
			$gfonts_data = thz_google_fonts_data();
			$gfonts_css_array = array();

			foreach($all_google_class_names as $gfcn){
		...
		if ( !empty( Thz_Doc::get('googlefont') ) ) {        
		...
		if ( !empty( Thz_Doc::get('cssinhead') ) ) {
			$add_css .= implode( Thz_Doc::get('cssinhead') );
		}
		...
		*/
		...
		$this->thz_compile_inline_css( $print_clean_css, $page_fonts );
	...
	public function _thz_shortcodes_css_files(){
		...
		if( function_exists('fw') && thz_fw_loaded() && thz_fw_active() ){
