...
function _thz_logo_css(){
	...
	if($logo_type == 'textual'){
		...
		$add_css .=	'.thz-logo-holder.type-textual .thz-logo h1{height:'.$logo_height.';}';
		$add_css .=	'.thz-logo-holder.type-textual .thz-logo h1 {'.thz_typo_css($f).'}';
		$add_css .= '.thz-logo-holder.type-textual .thz-logo h1 .site-description{';
		...
		$add_css .= '.thz-mobile-menu-holder .thz-logo-holder{width:'.$mlogo_width.';}';	
		$add_css .=	'.thz-mobile-menu-holder .thz-logo{min-height:'.$mlogo_height.';}';
		$add_css .=	'.thz-mobile-menu-holder .thz-logo-holder.type-textual .thz-logo h1{height:'.$mlogo_height.';}';
		...
		if($mc_t !==''){
			$add_css .=	'.thz-mobile-menu-holder .thz-logo-holder.type-textual h1{color:'.$mc_t.';}';
		}
		if($mc_s !==''){
			$add_css .=	'.thz-mobile-menu-holder .thz-logo-holder.type-textual h1 .site-description{color:'.$mc_s.';}';
		}	
		...
		if($sticky == 'active' ){
			...
			if($sc_t !==''){
				$add_css .=	'.thz-sticky-header.isvisible .thz-logo-holder h1{color:'.$sc_t.';}';
			}
			if($sc_s !==''){
				$add_css .=	'.thz-sticky-header.isvisible .thz-logo-holder h1 .site-description{color:'.$sc_s.';}';
			}
		}
		...
		$add_css .= '.thz-mobile-menu-holder .thz-logo-holder{width:'.$mlogo_width.';}';
		$add_css .=	'.thz-mobile-menu-holder .thz-logo{width:'.$mlogo_width.';min-height:'.$mlogo_height.';}';
		$add_css .=	'.thz-mobile-menu-holder .thz-logo-in a{height:'.$mlogo_height.';}';
		...
		if($svg_m !=''){
			$add_css .=	'.thz-mobile-menu-holder .thz-logo-holder.type-svg svg *{';
			if($svg_a =='fill' || $svg_a =='both'){
				$add_css .= 'fill:'.$svg_m.';';
			}
			if($svg_a =='stroke' || $svg_a =='both'){
...
function _thz_mainmenu_css(){
	...
	$tm_subul_bw = thz_akg('borders/top/w',$tm_subul_style,0);
	$sub_ul_pr = thz_akg('padding/right',$tm_subul_style,0);
	...
	if($ovsrc_o != '' || $ovsrc_t != '' || $ovsrc_b != ''){
		...
		if($ovsrc_t != ''){
			...
			$add_css .='#thz-wrapper .thz-search-overlay .thz-search-form .text-input{';
	...
	if(thz_detect_lateral_header()){
		...
		if($headers != 'offcanvas'){
			...
			if($actives_co  !=''){
				...
				$add_css .='background-color:'.$actives_co.';';
				$add_css .='}';
...
function _thz_footer_css(){
	...
	if('both' == $display_mode || 'footer' == $display_mode){
		...
		$footer_colors_print 		= thz_print_colorset($footer_colors,'.footer');
		...
		if( !empty( $footer_fonts )){
			...
			$add_css .='.thz-footer-menu{'.thz_typo_css(thz_get_option('fof/0/n',array())).'}';
		}
	}
...
* + .thz-content-50,* + .thz-content-40,* + pre,* + figure, * + form,* + table,.wp-caption,.gallery-caption,[class*=\'thz-text-column-\']{';
...
function _thz_site_spacings_css(){
	...
	$add_css .=	'padding-left:'.thz_property_unit($columns_spacing,'px').';';
	$add_css .=	'padding-right:'.thz_property_unit($columns_spacing,'px').';';
	$add_css .=	'}';
...
function _thz_cookies_consent_static(){
	...
	if ( 'active' == $active ) {
		...
		$add_css .= thz_print_button_css($button_data,'.thz-cookies-consent-button');
...
function _thz_posts_cat_static(){
	...
	if(!is_single()){
		...
		$prefix					= !empty(thz_get_option('cap/0',array())) ? 'cap/0/' : $prefix;
...
function _thz_related_static(){
		...
		if($rel_title =='show'){
			...
			if(!empty(thz_typo_css($r_t_f))){
				$add_css .='.thz-related-holder .thz-related-item-title,';
				$add_css .='.thz-related-holder .thz-related-item-title a{';
				$add_css .= thz_typo_css($r_t_f);
				$add_css .='}';	
			}
	...
	if ( class_exists( 'WooCommerce' ) ) {
		...
		if ( function_exists( 'is_woocommerce' ) ) {
			...
			if($imgh_picked =='custom'){
				...
				$add_css .='.thz-woo-products .thz-grid-item-media,';
				$add_css .='.thz-woo-products .thz-media-custom-size';
			...
			$add_css .= '.thz-woo-item-price{';
			$add_css .= $wooppbs_print;
			$add_css .= thz_typo_css($woopptf);	
			$add_css .= '}';
			
			if($wooppoc !=''){
				$add_css .='.thz-woo-item-price del{';
...
function thz_social_links_print($metrics,$key,$class,$css = false,$echo = true,$links = false){
	...
	$style = $links ? thz_akg('s',$metrics,'simple') : thz_get_theme_option($metrics.'/s','simple');
	
	if($css){
		
		$fsize = $links ? thz_akg('f',$metrics,14) : thz_get_theme_option($metrics.'/f',14);
		$ratio = $links ? (int) thz_akg('r',$metrics,2) : (int) thz_get_theme_option($metrics.'/r',2);
		...
		$shape = $links ? thz_akg('sh',$metrics,'square') : thz_get_theme_option($metrics.'/sh','square');
		...
		foreach ($socials as $link){
			...
			$html .='<a class="'.$class_name.'" href="'.esc_url($link['link']).'" target="_blank">';
...
function thz_responsive_print(){
	...
	if ( !empty( Thz_Doc::get('responsive') ) ) {
...
function thz_add_responsive_font($add_css){
	...
	foreach ($fonts as $font){
		...
		list($font_size,$unit) = preg_split('/(?<=[0-9])(?=[a-z]+)/i',$font[$selector]['font-size']);
...
function _thz_page_title_section_css() {
	...
	if($show_pagetitle){
		...
		if(!empty($page_title_margin)){
			$add_css .= '#'.$id_out.' .thz-pagetitle-title h5{'.$page_title_margin.'}';
		}
		...
		if(!empty($page_title_font )){
			...
			$add_css .= '#'.$id_out.' .thz-pagetitle-title h5{';
...
function thz_separators_css($option,$container){
	...
	if( 'triangle' == $separator_type || 'circle' == $separator_type ){
		...
		if(!empty($separator_icon)){
			$separator_css .= $container.' [class*=\'thz-ss-icon\']{line-height:'.$separator_size.'px;color:'.$separator_iconcolor.';}';
		}
			...
			if(!empty($separator_icon)){
				$separator_css .= $container.' [class*=\'thz-ss-icon\']{color:'.$separator_iconcolor.';}';
			}			
...
* Colorset CSS
*/
 function thz_print_colorset($colors_array,$container){
	...
	foreach ($colors_array as $property => $value){
		...
		if( $property == 'link_color'){
			$element_name = $container. ' a:not(.thz-button)';
			
			$css .=	$container." .thz-btn-none{color:".esc_attr($value).";}";
		}
		
		if( $property == 'link_hover_color'){
			$element_name = $container. ' a:not(.thz-button):hover';
