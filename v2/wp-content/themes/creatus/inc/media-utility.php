...
if( ! function_exists('thz_get_img_src')){
	...
	function thz_get_img_src( $data = false , $handle = false ){
		...
		$attachment_id = is_array( $data ) ? $data['attachment_id'] : $data;
 ...
 */
function thz_print_post_media_icons( $atts, $item_link, $media_data = array() ) {
	...
	if ( $show_icons == 'show' ) {
		...
		if ( $media_icon_show == 'show' && $attachment_id ) {
			...
			$mfp_href      = '#';
			$magnific_data = thz_lightbox_data() . ' data-mfp-title="' . $img_title . '" data-modal-size="large"';
			...
			$icons_output .= '<a class="thz-hover-icon ' . thz_sanitize_class( $icon_classes ) . ' thz-lightbox ' . $mfp_type . '"';
 ...
 * @return array
 */
function thz_get_post_media( $placeholder = true, $featured = true ) {
